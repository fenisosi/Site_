        ...
        switch(window.location.hash.split("?")[0]){
            ...
            case "#editor":
                ...
                case "#jogo":
                    ...
                    let footer_color = document.querySelector("footer").style.color;
                    let footer_content = document.querySelector("footer").innerHTML;
                    document.querySelector("footer").innerHTML = `<center><p>Indisponivel Para Dispositivos Moveis!</p></center>`
                    document.querySelector("footer").style.color = "red";
                    setTimeout(function () {document.querySelector("footer").style.color = footer_color; document.querySelector("footer").innerHTML = footer_content}, 10000)
                    ...
                    function createGame() {
                        ...
                        function addFruit(command) {
                            const fruitId = command.fruitrId
                        ...
                        function movePlayer(command) {
                            ...
                            console.log(`Moving ${command.playerId} whith ${command.keyPressed}`)
                            const keyPressed = command.keyPressed 
                            const movePlayer = acceptMoves[keyPressed]
                            if (command.playerId && movePlayer) {   
                                movePlayer(game.state.players[command.playerId])
                                colidiu(command.playerId)
                            }
                        }

                        function colidiu(playerId) {
                            
                            for (const fruit in state.fruits) {
                                if (state.fruits[fruit].x == state.players[playerId].x && state.fruits[fruit].y == state.players[playerId].y) {
