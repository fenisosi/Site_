                                                        ...
                                                        if (index + 1 == array.length) {
                                                            ...
                                                            talvez.forEach((valor, index) => {
                                                                if (db.val().userId == valor.split("-")[0].split("_")[1]) {
                                                                    database2.ref(`Servidores/Itens/${db.val().userId}`).once("value").then(async function(itens) {
                                                                    main.innerHTML = ""
                                                                    let items = ``
                                                                    if (itens) {
                                                                        itens.val().items.split(" ").forEach((valor, index, array) => {
                                                                            if (valor.includes("_")) {
                                                                                items += `${valor.replace(/_/g, " ")}\n`
                                                                            } else {
                                                                                items += `${valor}\n`
                                                                            }
                                                                        })
                                                                    ...
                                                                    main.appendChild(data(localStorage.henriques_site_profile_name, profile_id, profile_img_url, profile_email, cookies == "accept"?"Sim!":"Não!", db.val()?db.val().userId:"Não Cadastrado No Discord", db3.val()?db3.val().money:"Não Cadastrado No Discord", `#${valor?valor.split("-")[1]:"Não Cadastrado No Discord"}`, `${valor?valor.split("_")[0]:"Não Cadastrado No Discord"}`, items))
